...
def main():
    # --- ログファイル設定 ---
    log_dir = "logs"
    os.makedirs(log_dir, exist_ok=True) # logsディレクトリがなければ作成
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file_name = f"shift_generator_log_{timestamp}.txt"
    ...
    try:
        with open(log_file_path, 'w', encoding='utf-8') as log_file_obj: # 変数名を log_file から log_file_obj に変更
            sys.stdout = log_file_obj
            sys.stderr = log_file_obj
            
            print(f"--- Shift Generator Script Start ({timestamp}) ---")
            print(f"ログファイル: {os.path.abspath(log_file_path)}") # 絶対パスも出力
        ...
        sys.stdout = original_stdout # エラー時も標準出力を戻す
        sys.stderr = original_stderr
        current_time = datetime.now().strftime("%Y%m%d_%H%M%S")
        print(f"[{current_time}] Shift generator script encountered a global error: {e_global}")
        import traceback
        traceback.print_exc()
        # ログファイルにもエラーを書き込む (リダイレクトされていれば自動で書き込まれる)

    finally:
        sys.stdout = original_stdout
        sys.stderr = original_stderr
        if 'log_file_path' in locals(): # log_file_pathが定義されていれば
            print(f"ログは {os.path.abspath(log_file_path)} に保存されました。")
        else:
            print("ログファイルパスが未定義のため、ログ保存場所は不明です。")
